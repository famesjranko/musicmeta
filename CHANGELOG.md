...
### Fixed
- `country` now reports ISO 3166-1 alpha-2: Wikidata no longer emits a name or a raw Q-id, iTunes no longer alpha-3 (`USA` is `US`), `UK` is `GB`. Cached entries keep the old value until refresh
