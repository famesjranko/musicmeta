...
### Changed
- `ExternalLink.label` now carries the site the URL points at (its host, `www.` dropped) where the upstream names none: a consumer showing `type` can show `label` instead of "social network" twice
...
- `RELEASE_EDITIONS` applies the same year check to a supplied release-*group* id, and answers nothing when it fires: editions have no name route to recover by, so the identifier was the whole answer
...
- `SimilarArtist.matchScore`'s 1.0 top entry and descending sort are documented as facts about the merge, not a served result: catalog filtering can remove the top entry or reorder the list after it
