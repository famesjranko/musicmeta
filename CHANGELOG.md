...
- `ExternalLink.url` documented as the upstream's URL verbatim, which may be `http://`: the library never rewrites the scheme, so an https-only policy is the consumer's to enforce
