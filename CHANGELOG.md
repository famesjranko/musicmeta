...
### Changed
- **Build: the release version is declared once, in root `gradle.properties`** — the three modules (`musicmeta-core`, `musicmeta-android`, `musicmeta-okhttp`) previously each declared `version` in their own `build.gradle.kts`, so a release edited the same string in three places and nothing structurally stopped them drifting (the footgun behind the 0.10.0 mismatch on `dev`). A single `version` in the root `gradle.properties` is now inherited by every module, making cross-module drift unrepresentable rather than merely detected. `group` stays per-module — it has never drifted and the `coordinates(…)` calls carry it as a separate literal. The published Maven coordinates are unchanged: the generated POMs are byte-for-byte identical before and after, and the existing version guards (#13 in `publish.yml`, `release-readiness` in `build.yml`) read the effective Gradle version and stay green as backstops. No public-API or consumer-visible change. (#37)
...
- **CI: GitHub Actions bumped off the deprecated Node 20 runtime** — every `uses:` reference across all four workflows (`build.yml`, `publish.yml`, `api-drift.yml`, `provider-drift.yml`) now resolves to an action major whose manifest declares the `node24` runtime: `actions/checkout` v4→v7, `actions/setup-java` v4→v5, `gradle/actions/setup-gradle` v4→v6, `actions/upload-artifact` v4→v7 (v6 is the first upload-artifact major off `node20` and v7 is current; v5 is still `node20`). The v4 majors all declared `using: node20`; the runners flipped to Node 24 by default on 2026-06-16 and force-ran them there, emitting a deprecation annotation on every run — including the Maven Central tag path — and the `ACTIONS_ALLOW_USE_UNSECURE_NODE_VERSION` shim expires when Node 20 is removed from the runners in the fall of 2026. `provider-drift.yml` stays pinned by commit digest because it holds provider secrets; its three pins were updated to the new versions' commit SHAs (checkout `3d3c42e` v7.0.1, setup-java `03ad4de` v5.6.0, setup-gradle `3f131e8` v6.2.0), while the other three files stay tag-pinned as before. No change to what any workflow builds, tests, publishes, or writes — job graph, triggers, permissions, secret scope and published artifacts are all identical before and after. (#10)
...
### Added
- **CI: scheduled provider-API drift watch** — a daily (`workflow_dispatch`-able) workflow (`provider-drift.yml`) runs the gated E2E suite against the live third-party APIs and files — or updates in place — a single tracking issue when they drift, auto-closing it once the suite is clean again. This is the inverse of `api-drift.yml` below: that one watches musicmeta's own published ABI and protects consumers from us, while this one watches MusicBrainz, Deezer, Last.fm, Discogs and the rest changing their JSON or endpoints underneath us. That failure is silent by construction, because every parser uses `org.json`'s `opt*` family and a renamed field yields a default rather than throwing — so drift arrives as a *successful* enrichment containing nothing, and then sits in consumers' caches for up to the TTL. v0.9.1 shipped exactly that (ListenBrainz `track_name` → `recording_name`). It has to be scheduled rather than change-triggered because it arrives with zero commits to this repo: a provider deploys on their own schedule while our tree is untouched and every other gate is green. Never gating, per the rule that third-party rate limits must not be able to block a merge. Missing credentials are reported as misconfiguration rather than as drift, a single failure is retried once to absorb a transient 429, and the run is scoped to the `e2e` package so an ordinary unit-test regression cannot masquerade as provider drift. Actions are SHA-pinned rather than tag-pinned because this job holds provider secrets. (#16)
...
### Breaking Changes
> Permitted under the 0.x semver carve-out (see `CLAUDE.md` / `STORIES.md` 2026-07-21): during `0.x`, **minor** releases may break as long as the break is documented here and visible in the reviewed `.api` diff. This is an ABI removal — the removed types are gone at compile time for any consumer who imported them. They were public by omission, never by design: each existed only to serve the `*Provider` in its own package. The `.api` baseline shrinks by ~1350 lines, and `apiCheck` now stays green through a rename inside `provider/*/` or `http/` internals — which is the point.
- **Provider internals are now `internal`** — the `*Api` (11), `*Mapper` (11) and `*Models` response data classes (49) behind every provider, plus `MusicBrainzParser`, are no longer part of the published API. This includes types such as `DeezerApi`, `DiscogsMapper`, `DeezerAlbumResult`, `MusicBrainzArtist`, `DiscogsRelease`, `LastFmTrackInfo`, etc. Consumers construct and register the public `*Provider` classes (`DeezerProvider`, `MusicBrainzProvider`, …); those are unchanged and remain the intended entry point.
- **`http/` infrastructure is now `internal`** — `CircuitBreaker` (and its `State` enum) is hidden. `HttpClient`, `HttpResult`, `HttpResponse`, `DefaultHttpClient` and `RateLimiter` stay public: they are the contract a consumer implements (`HttpClient`) or supplies (`RateLimiter`, passed to every provider constructor).
- **`engine/` mergers and synthesizers are now `internal`** — `ArtworkMerger`, `GenreMerger`, `SimilarArtistMerger`, `SimilarTrackMerger`, `TopTrackMerger`, `TimelineSynthesizer` and `GenreAffinityMatcher` are hidden. The `ResultMerger` and `CompositeSynthesizer` **interfaces** stay public — they are the extension points on `EnrichmentEngine.Builder` (`addMerger`, `addSynthesizer`), so consumers implement their own without depending on the built-in strategies.
- **`SimilarAlbumsProvider(DeezerApi)` constructor replaced** — its only public constructor took the now-internal `DeezerApi`. It now exposes `SimilarAlbumsProvider(httpClient: HttpClient, rateLimiter: RateLimiter = RateLimiter(100))`, matching the other providers, and builds the `DeezerApi` internally. A consumer who constructed it with a `DeezerApi` switches to the HTTP-client form.
- **`ProviderChain` constructor is now `internal`** — its default `circuitBreakers` parameter exposed the now-internal `CircuitBreaker`. The class stays public and is still reachable via `ProviderRegistry.chainFor(type)`; only direct construction is withdrawn (nothing outside the engine constructed it).
