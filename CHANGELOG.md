...
- A failed MusicBrainz alias lookup now costs one retry ladder per `enrich()`, not one per reading provider: an outage no longer spends most of `enrichTimeoutMs` re-asking a down endpoint
