...
- Kotlin written through Bash — `sed -i`, heredocs, generator scripts — is formatted again, via a `Stop` hook that sweeps every dirty file at the end of a turn. The `PostToolUse` hook only ever saw files a payload named, so these were skipped silently. Target selection has a self-test, because a hook that formats nothing exits 0 exactly like one that works (#59)
