...
`MusicBrainzTrackEnrichment.pickBestRecording` and `MusicBrainzReleaseRanking.pickBestRelease` rank a pool
...
`MusicBrainzSearchTitleProvenanceTest`). `MusicBrainzSearchOutcome.searchProvenance` compares the
...
(`MusicBrainzArtistEnrichment`, `MusicBrainzAlbumEnrichment`, `MusicBrainzTrackEnrichment`) —
...
- **Payload staleness is recoverable; identity staleness is not.** A memo keyed by MBID serves at
  worst an out-of-date payload for an entity already resolved. `MusicBrainzAlbumEnrichment`'s album-search
...
- `MusicBrainzTrackEnrichment.pickBestRecording` — 42 comment lines, 17 code, the worst body-bearing case
