...
| Core dependencies | `scripts/checks/check_core_dependencies.py` | every `api`/`implementation`/`compileOnly`/`runtimeOnly` in `musicmeta-core/build.gradle.kts` is on the script's allowlist with a reason — the `ARCHITECTURE.md` "dependency-minimal JVM" invariant, which was prose until this check and is invisible to every other gate (a transitive is not part of the ABI). Core only, deliberately: the adapters exist to bring OkHttp and Room. It reads the build script, not the resolved graph, so a transitive arriving under an allowed dependency and a version bump are both invisible |
