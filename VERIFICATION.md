...
| Private companion constants | `scripts/checks/check_private_companion_consts.py` | no bare `const val` inside a `private companion object` in the published modules' main sources. Kotlin honours the private companion and the JVM does not, so one compiles to a `public static final field` on the enclosing class: the dump publishes a constant no Kotlin caller can see, and deleting it later is a break. `private const val` in the same companion does not leak and reads the same at every call site. Nothing else catches it — `apiCheck` freezes the leaked line rather than objecting to it, detekt has no rule keyed on companion visibility, and `explicitApi()` is already satisfied. It reads modifiers, never intent: a constant that *should* be published is written in a public companion and is invisible to this |
...
| Route pin coverage | `scripts/checks/check_route_pin_coverage.py` | every upstream route — a non-private `suspend fun` in a `provider/<name>/*Api.kt` that reaches an `httpClient.fetch*`/`post*` call through same-file calls — is either tied to a `SchemaTarget` (by `route =` matching its name, or by `url =` calling a URL builder the route itself calls) or named in the script's allowlist with a reason, and every `httpClient` call that no route reaches is named in its second list. Both lists reject entries they have outlived. The reachability closure is same-file, and most routes are allowlisted rather than pinned, so what it enforces is that a *new* route arrives with the decision made either way |
...
| Commit attribution | `scripts/checks/check_commit_attribution.py` | no commit this branch adds to its base credits an assistant — as its author or committer identity, or in a `Co-authored-by:` naming one, a `Claude-Session:` link, or a "Generated with" footer. The identity half is not theoretical: a hosted agent session's container arrives with `user.email` set to an assistant's address, and only squash-merging (which rewrites the author to the PR's) has been hiding it. It reads the branch's commit messages, not the tree, so CI checks out unshallow for it and it fails rather than passes when the base will not resolve. There is no bot allowlist: `dependabot[bot]` and `github-actions[bot]` match nothing it looks for, while a `[bot]` identity that names an assistant is the finding. It reads trailers, never prose — a body discussing the tool passes, which is what lets this row exist — and it cannot see the squash body GitHub composes at merge, or a PR title, description or issue comment; see "Known gaps" |
...
| Build | `./gradlew build -Pmusicmeta.rerunTests` | compile, all unit tests, `apiCheck` against `api/*.api`. The property makes every `Test` task neither up-to-date nor cacheable for this run: the build cache is on and its directory is shared by every checkout on the machine, so without it `./check` can report green having executed no test at all, against reports another tree wrote on another day |
...
| Consumer canary | `demo-cli/` and `demo-web/` composite builds | an external consumer still compiles, and both demos' test source sets run — `demo-web/`'s reading of a profile off the wire, `demo-cli/`'s command parsing and rendering. Neither reaches a provider, so what they prove is that the demo agrees with the surface it compiled against, never that the surface answers |
| Test results are from this run | `scripts/checks/check_test_freshness.py` | every module with test sources produced a JUnit report, and no report predates the run's start. It is what makes the forcing above non-silent: a property that stops being passed, or a `withType<Test>` block someone unpicks, otherwise hands back the inherited green this gate exists to refuse. It reads timestamps, never outcomes — a fresh report of a suite that ran nothing still passes, which is the gap below. `schemaPin`'s results are excluded (it is never part of `check`), and `--skip-demo` excludes the demo modules, whose reports are then legitimately older |
| Doc samples | `scripts/checks/check_doc_samples.py` + `docs-samples/`/`docs-samples-android/` composite builds | every ```` ```kotlin ```` fence in `docs/guides/*.md` compiles against the real API, or carries a `<!-- no-compile: <reason> -->` marker with a mandatory reason; compiled per guide, in reading order, as one narrative, not one fence at a time — see "Known gaps". One extractor, two targets: every guide but `android.md` compiles as a plain JVM module, `android.md` compiles against a real `com.android.library` build (Room/Hilt/WorkManager). 101 of 143 non-`android.md` fences and 12 of 13 `android.md` fences compile today |
...
- **Nothing reads the text that leaves this machine without being a commit.** `CLAUDE.md` forbids
  ...
  commits, and `check_commit_attribution.py` reads only the latter. Two of those are where the
  attribution actually tends to appear — a "Generated with Claude Code" footer belongs to a PR
  description, not a commit message — so the gate covers the surface that happened to be hit here,
  not the surface the rule names. GitHub composing a squash body at merge is the same gap from the
  other end: the gate reads the branch's commits, and the extra `Co-authored-by:` the squash UI
  harvests from them is written after every check has passed. Both rest on `.claude/settings.json`
  turning the behaviour off at the source, which is one repository's setting on one tool.
...
- **Most routes are pinned to nothing, and the check that counts them cannot say which are merely
  unattributed.** `check_route_pin_coverage.py` enumerates every route and requires each to
  be pinned or allowlisted, so a new one cannot arrive silently — but a pin is tied to a route only
  by name or by a shared URL builder, and most routes build their URL inline, so being on the
  allowlist means "no mechanical key", not always "no pin". Last.fm's `getArtistInfo` is the case
  that shows the difference: it is pinned, and one level of builder indirection is enough to lose
...
- **The freshness gate proves the tests ran, not that they ran anything.** It reads each JUnit
