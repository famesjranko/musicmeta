...
| Core dependencies | `scripts/checks/check_core_dependencies.py` | every `api`/`implementation`/`compileOnly`/`runtimeOnly` in `musicmeta-core/build.gradle.kts` names a version-catalog accessor on the script's allowlist, with a reason — a declaration in any other form (a raw `"group:name:version"` coordinate, named arguments, `platform(...)`, `project(...)`) is reported rather than skipped, since a line the parse does not recognise is a dependency added in silence — the `ARCHITECTURE.md` "dependency-minimal JVM" invariant, which was prose until this check and is invisible to every other gate (a transitive is not part of the ABI). Core only, deliberately: the adapters exist to bring OkHttp and Room. It reads the build script, not the resolved graph, so a transitive arriving under an allowed dependency and a version bump are both invisible |
