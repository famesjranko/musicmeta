...
| Explicit API | Kotlin's `explicitApi()` on `musicmeta-core`, `musicmeta-okhttp`, `musicmeta-android` | every declaration reaching the published surface states its visibility and its return type, or the compile fails. It runs inside the compile, so every build — `check`, `check --fast`, CI, a plain `./gradlew build` — enforces it. What it buys a consumer: nothing lands in `api/*.api` by inference, so a member that was meant to be internal fails at the moment it is written rather than being caught, or not, by whoever reads the dump diff; and a public signature's types are readable in the `.kt` rather than reconstructed from the dump's erased descriptors. Main source sets only: tests, `testFixtures`, `e2e/`, the demos and `docs-samples/` are unaffected, and so are `internal` and `private` declarations anywhere. It reads declarations, never intent — it forces a visibility to be written, it cannot tell that the one written is the right one |
