...
| Test shape | `scripts/checks/check_test_shape.py` | every `@Test` body has `// Given —`/`// When —`/`// Then —`, each on its own line with a dash and a real clause — test sources only |
