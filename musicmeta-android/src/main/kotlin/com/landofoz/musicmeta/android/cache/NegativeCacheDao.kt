...
@Dao
public interface NegativeCacheDao {

    @Query(
        "SELECT * FROM negative_cache WHERE entity_key = :entityKey AND enrichment_type = :type AND expires_at > :now LIMIT 1",
    )
    public suspend fun get(entityKey: String, type: String, now: Long): NegativeCacheEntity?

    @Insert(onConflict = OnConflictStrategy.REPLACE)
    public suspend fun insert(entity: NegativeCacheEntity)
