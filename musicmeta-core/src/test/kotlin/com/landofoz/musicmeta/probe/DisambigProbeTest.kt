 ...
 */
// ---- ARM BLOCK START ----
private const val ARM_NAME = "mbbatch"
...
/**
 * Arm D bills one `/ws/2/artist?query=arid:… OR arid:…` request per *answer* that needs any lookup
 * at all, so a second unknown MBID in the same enrich is free.
 *
 * `calls` is the ledger: one line per request this arm would have made. The harness clears it
 * between artists, so "already billed this answer" is a lookup into the current artist's lines.
 */
private fun bill(calls: MutableList<String>, mbid: String) {
    val existing = calls.indexOfFirst { it.startsWith("GET /ws/2/artist?query=arid:") }
    if (existing < 0) {
        calls += "GET /ws/2/artist?query=arid:$mbid"
    } else {
        calls[existing] = calls[existing] + " OR arid:$mbid"
    }
}
