 ...
 */
private const val ARM_NAME = "nocap"
...
private fun mergeArm(providerLists: List<List<SimilarArtist>>): List<SimilarArtist> =
    SimilarArtistMerger.mergeArtists(providerLists.flatten())
...
class MergerNormalisationProbeTest {
    ...
    fun `known single-provider case matches hand-computed scores`() {
        ...
        // Arm A (nocap): no per-provider normalisation, but the final rescale-by-own-max divides
        // every score by the group's top raw score (0.9), so the top lands at exactly 1.0.
        org.junit.Assert.assertEquals(1.0f, merged[0].matchScore, EPS)
        org.junit.Assert.assertEquals(0.6f / 0.9f, merged[1].matchScore, EPS)
        org.junit.Assert.assertEquals(0.2f / 0.9f, merged[2].matchScore, EPS)
    }
