 ...
 */
private const val ARM_NAME = "rank-rescale"
...
/** Flip to TRUNCATED_TO_COMMON for the "run both ways" rank-N-basis comparison the plan requires. */
private val RANK_N_BASIS = com.landofoz.musicmeta.engine.SimilarArtistMerger.RankNBasis.OWN_LENGTH

private fun mergeArm(providerLists: List<List<SimilarArtist>>): List<SimilarArtist> =
    SimilarArtistMerger.mergeArtists(providerLists, RANK_N_BASIS)
...
class MergerNormalisationProbeTest {
    ...
    fun `known single-provider case matches hand-computed scores`() {
        ...
        // Arm C (rank-rescale): rank formula 1 - i/n (n=3) then rescale by the merged list's own
        // max — for one contributor the rank-normalised top is already 1.0, so rescale is a no-op.
