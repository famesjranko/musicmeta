...
import com.landofoz.musicmeta.engine.nameMatchTier
import com.landofoz.musicmeta.engine.withAliasPool
...
class NonLatinCoverageProbe {
    ...
    private val useLatinNames get() = armName == "a" || armName == "c"
    private val usePool get() = armName == "b" || armName == "c"
    ...
    fun `non-latin workload across deezer itunes and discogs`() = runBlocking {
        ...
        for (item in WORKLOAD) {
            val identity = if (useLatinNames || usePool) resolveIdentity(mbApi, counter, item) else null
            val requestArtist = if (useLatinNames) identity?.latinName ?: item.artist else item.artist
            for ((provider, type) in providersFor(item, deezer, itunes, discogs)) {
                val request = requestOf(item, requestArtist)
                ProbeTrace.reset()
                val before = counter.count(providerHost(provider.id))
                val startedAt = System.currentTimeMillis()
                val result = withPool(identity?.pool.orEmpty()) { provider.enrich(request, type) }
                val elapsed = System.currentTimeMillis() - startedAt
                val requests = counter.count(providerHost(provider.id)) - before
                rows += row(item, provider.id, type, identity, result, elapsed, requests)
            }
        }
    ...
    private fun row(
        ...
        return listOf(
            ...
            identity?.mbid.orEmpty(),
            (identity?.mbid == item.artistMbid).toString(),
            identity?.latinName.orEmpty(),
            requests.toString(),
            elapsedMs.toString(),
            (detail + " tiers=" + ProbeTrace.aliasTiers.distinct()).take(PICK_DETAIL_CHARS).replace('\t', ' ').replace('\n', ' '),
        ).joinToString("\t")
    }
    ...
    /**
     * The identity step both name-bearing arms need: MusicBrainz's own artist search, ranked the way
     * the enricher ranks it, and the alias pool that comes back on the hit.
     */
    private suspend fun resolveIdentity(api: MusicBrainzApi, counter: CountingHttpClient, item: Item): Identity? {
        val before = counter.count(MB_HOST)
        ...
        val pool = listOf(AlternativeName(best.name, official = true)) + best.alternativeNames()
        return Identity(
            mbid = best.id,
            canonical = best.name,
            pool = pool,
            latinName = bestLatinName(best.name, best.alternativeNames()),
            requests = counter.count(MB_HOST) - before,
        )
    }

    /** Canonical when it is Latin script, else the highest-tier Latin alias, else the name as given. */
    private fun bestLatinName(canonical: String, aliases: List<AlternativeName>): String {
        if (isLatin(canonical)) return canonical
        val latin = aliases.filter { isLatin(it.name) }
        return latin.firstOrNull { it.official }?.name ?: latin.firstOrNull()?.name ?: canonical
    }

    private fun isLatin(name: String): Boolean {
        val letters = name.filter { it.isLetter() }
        return letters.isNotEmpty() && letters.all { Character.UnicodeBlock.of(it) in LATIN_BLOCKS }
    }
    ...
    private suspend fun <T> withPool(pool: List<AlternativeName>, body: suspend () -> T): T {
        if (!usePool) return body()
        return withAliasPool(pool, body)
    }

    private data class Identity(
        val mbid: String,
        val canonical: String,
        val pool: List<AlternativeName>,
    ...
    private companion object {
        ...
        const val HEADER =
            "arm\titem\tkind\tprovider\ttype\toutcome\tpickedArtist\tpickedDetail\t" +
                "identityMbid\tidentityCorrect\tlatinName\trequests\tms\tdetail"
