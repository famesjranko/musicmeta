...
/**
 * Pins `scripts/probes/mb-search-4xx-probe.sh`'s live finding (2026-08-12: every case below sent
 * to MusicBrainz answered 200, never a 4xx) as a property of [MusicBrainzApi.buildQuery]'s escaping
 * rather than a fact that decays with the network: every Lucene special character is
 * backslash-escaped before the phrase is quoted, the quote itself cannot be broken out of, and the
 * whole expression is URL-encoded — so nothing a title/artist can carry reaches MusicBrainz as
 * unescaped Lucene syntax or an unterminated phrase. `.scratch/mb-4xx-as-empty-pool/spec.md`
 * question 1, answered: outcome (a), provably unreachable.
 */
